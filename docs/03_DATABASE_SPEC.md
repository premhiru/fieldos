...
`WhatsAppGroupParticipant` stores the latest roster observed from an active project-mapped group. Synchronization marks removed members inactive only after a complete authoritative snapshot and updates project participation, but it does not create memberships or `ProjectAccess` rows.
...
`RecommendationDelivery` records recipient identity, account, recommendation, status, attempt count, defer/expiry times, atomic send claim, confirmation actor, provider message key, error summary, and timestamps. Its recommendation-recipient idempotency key is unique. `RecommendationResponse` stores the exact parsed command, sender, inbound message key, outcome, and optional rejection reason; inbound keys are unique per delivery so provider replays cannot apply a command twice. `Recommendation.snoozedUntil` records recommendation-level snoozing.

`WhatsAppInvitation` stores only a SHA-256 token hash. One active invitation is allowed per person and project. It expires, is single-use, records the confirmed sender and delivery attempts, and grants a scoped project role only after authenticated acceptance. `WhatsAppOperationAudit` records security-relevant delivery, command, identity, participant, and invitation operations with organization and project indexes. Audit metadata must not contain secrets, raw session material, or provider payload dumps.
